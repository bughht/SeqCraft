...
| | |
|---|---|
| [`01_build.ipynb`](t2prep_gre_2d/01_build.ipynb) | What a T2 preparation is made of — fifteen hard pulses at one B1, an MLEV-4 composite refocusing train, a composite tip-up — and the two instants `prep_time_s` is measured between. Then the emitted block checked for interval symmetry and for the absence of any gradient while the magnetisation is transverse, and a composition with a shipped `GRE2D`. Four `.seq` files. **Needs nothing but `seqcraft`.** |
| [`02_simulate_and_reconstruct.ipynb`](t2prep_gre_2d/02_simulate_and_reconstruct.ipynb) | A known T2 recovered from the slope of `ln S` against `prep_time_s`, within 2 % from 40 to 250 ms — and the control that matters: T2\* swept by a factor of five moves the answer by 3 %. Then where the residual bias comes from. **Needs `MRzeroCore` and `torch`.** |
...
```text
90x  ->  refocusing train  ->  tip-up  ->  spoiler  ->  any readout at all
     |<------ prep_time_s ------>|
```
