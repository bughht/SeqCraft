...
`IRPrep` shipped — two consumers, and a different `rf.use`, so a new `preparation/` folder. The two
...
salvage/      physics lifted out of the deleted library, and the withdrawn MATLAB
              frontend parked whole; not packaged, not imported
