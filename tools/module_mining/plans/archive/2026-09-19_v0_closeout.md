...
**Important:** nothing here is actionable now. The v0 close-out is done; the next-phase sections below have been superseded by [`current/roadmap.md`](../current/roadmap.md) and are kept because they record *why* those directions were chosen, which the roadmap does not repeat.
